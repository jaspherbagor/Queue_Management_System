        ...
        if (!empty($tokens)) {
            ...
            foreach ($tokens as $token) {
                ...
                if ($token->status != 0 || !empty($token->updated_at)) {
                    $options .= "<a href=\"" . url("admin/token/recall/$token->id") . "\"  class=\"btn btn-info btn-sm btn-call\" onclick=\"return confirm('Are you sure you want to recall?')\" title=\"Re-call\"><i class=\"fa fa-phone\"></i></a>";
                }
                if ($token->status == 0) {
                    $options .= "<button type=\"button\" data-toggle=\"modal\" data-target=\".transferModal\" data-token-id='{$token->id}' class=\"btn btn-primary btn-sm btn-transfer\" title=\"Transfer\"><i class=\"fa fa-exchange\"></i></button>
                        <a href=\"" . url("admin/token/stoped/$token->id") . "\"  class=\"btn btn-warning btn-sm btn-stop\" onclick=\"return confirm('Are you sure you want to stop?')\" title=\"Stop\"><i class=\"fa fa-stop\"></i></a>";
                }
                ...
                $options .= "<button type=\"button\" href=\"" . url("admin/token/print") . "\" data-token-id='$token->id' class=\"tokenPrint btn btn-default btn-sm btn-print\" title=\"Print\"><i class=\"fa fa-print\"></i></button>
                    <a href=\"" . url("admin/token/delete/$token->id") . "\" class=\"btn btn-danger btn-sm btn-delete\" onclick=\"return confirm('Are you sure you want to delete?');\" title=\"Delete\"><i class=\"fa fa-trash\"></i></a>";
                $options .= "</div>";
